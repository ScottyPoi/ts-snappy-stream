...
export class CompressStream extends Transform {
  ...
  private async _asyncTransform(chunk: Uint8Array, callback: (error?: Error | null) => void): Promise<void> {
    const processChunk = async (startFrom: number): Promise<void> => {
      ...
      try {
        const compressed = await compress(Buffer.from(bytesChunk));
  ...
  private async _syncTransform(chunk: Uint8Array, callback: (error?: Error | null) => void): Promise<void> {
    try {
